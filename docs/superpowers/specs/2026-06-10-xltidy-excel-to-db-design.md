...
| `xltidy consolidate <dir|glob> --spec spec.yaml [--out merged.parquet]` | 통합 + 드리프트 |
...
1. `period` 출처 기본값: 파일명 패턴 우선인가, 특정 셀 지정 우선인가? (현재 설계는 둘 다 지원, 스펙에서 택)
2. 통합 출력 스키마: 완전 long(metric/value)으로 강제할지, index_columns는 wide 유지할지 — 도메인 확인 필요.
3. 드리프트 발생 시 동작: 기본은 "플래그 후 중단"인가 "best-effort 진행 + 경고"인가?
