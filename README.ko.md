...
<p align="center"><sub>🚧 <b>Status:</b> 초기 버전(v0.2.x)입니다 — CLI는 끝까지 동작하며 API는 바뀔 수 있어요. 이슈·피드백 환영!</sub></p>
...
- **워크북 1개 → 모든 표를 쌓은 통합 `db.csv` 1개** (`--per-table`이면 표별 파일) — *"엑셀 1개 = DB 1개"*.
...
- **버전 통합**: 월/분기 파일을 `version` 차원(파일명 매칭 토큰, 예: `2024Q1`)으로 한 시계열에 쌓고, 새 파일이 양식과 어긋나거나(컬럼 이름변경·시트 누락·영역 이동) 두 파일이 **같은 / 미해석 version**으로 해석되면 플래그합니다.
...
0) moa sheets <파일>  ─▶  모든 시트(숨김 포함) 나열  ─▶  [사용자가 DB화할 시트 선택]
   ...
   kind=pivot:  pivot (COM, 필터 해제) ────────────────────────────────▶ TableSpec   (LLM 건너뜀)
                                                            │
   각 월/분기 파일 + TemplateSpec ─ apply ─▶ {table: tidy long(+version)} + reconcile + verify
                                                            │
   여러 파일 ─ consolidate ─▶ version 쌓기 + 드리프트/version 검사 ─▶ 통합 db.csv/.parquet 1개 (--per-table이면 표별 파일)
```
...
#    피벗 시트: 스펙에 kind: pivot + pivot_name + version 만 작성
#    (추출 시 필터 자동 해제)
...
`moa consolidate` 는 **파일별 진행상황**(`[2/12] processing ... OK -> 1,234 rows, version=2024Q2`)과 최종 **요약 테이블**(파일 / 상태 / 행수 / version / 비고)을 출력합니다.

정합성 불일치(소계 ≠ 합, 피벗 데이터 ≠ 총합계), 드리프트(헤더 이름변경, 선택 시트 누락), version 충돌(두 파일이 같은/미해석 version으로 해석)이 보고되며, `--on-drift stop`이면 어긋난 파일은 적재되지 않습니다.
...
src/moa/                        (패키지는 `moa`로 import; CLI 명령은 `moa`)
  ...
  dbio.py         write_tables -> 통합 db.csv/.parquet 1개(기본) 또는 표별 파일
  consolidate.py  detect_drift (시트/컬럼/영역) + version 충돌 가드 + consolidate
