...
CLI 명령은 <code>moa</code> (레거시 <code>xltidy</code> 명령도 동작하며, 패키지는 <code>xltidy</code>로 import).</sub></p>
...
- 무인 추론은 사내 **Qwen** 백엔드(`--backend qwen`)를 사용하며 `XLTIDY_QWEN_BASE_URL`, `XLTIDY_QWEN_API_KEY`, `XLTIDY_QWEN_MODEL`로 설정합니다.
...
```
src/xltidy/                        (패키지는 `xltidy`로 import; CLI 명령은 `moa`)
