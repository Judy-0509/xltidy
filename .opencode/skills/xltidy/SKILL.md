...
1. **스펙 작성(양식당 1회)**:
   ...
   - 표 시트: `xltidy infer <파일> --sheet <시트> --backend agent` → 인코딩을 읽고 `specs/<id>.yaml`의 해당 sheets[] 항목을 작성. 골격이 필요하면 `xltidy sample-spec`로 유효한 YAML 예시를 받아 채운다. 다중헤더는 값열마다 column_semantics 한 항목으로 평탄화. 숫자는 #num이니 좌표·구조만. **대형 시트는 자동으로 값-있는 행 기준 머리 40 + 꼬리 10행만 프롬프트에 싣는다(`--head/--tail`로 조절). 인코딩의 `DATA ROWS: a..b`가 표의 진짜 끝이니 region 끝 행은 b로 잡을 것.**
   - 피벗 시트: 추론하지 말고 `kind: pivot` + `pivot_name`(없으면 null) + `period`만 적는다.
2. **검증**: `xltidy spec-validate specs/<id>.yaml --against <파일> --sheet <시트>`
3. **단일 적용(검증 포함)**: `xltidy apply specs/<id>.yaml --file <파일> --out-dir out/<엑셀-stem-슬러그> --format csv --verify --sample 100` (출력 폴더는 워크북별 = 엑셀 파일명 기준, 1엑셀=1DB) → reconcile/verify FAIL이면 스펙(소계/영역/피벗명) 수정.
